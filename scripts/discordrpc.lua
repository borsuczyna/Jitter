...
discordRPC.topText = "In main menu"
discordRPC.downText = "Selecting language"

function discordRPC.init()
    discordRPC.initialize("905043879647146014", true)
    discordRPC.now = os.time(os.date("*t"))
    presence = {
        state = discordRPC.downText,
        details = discordRPC.topText,
        largeImageKey = "logo",
        startTimestamp = discordRPC.now,
    }
...
function discordRPC.update()
    if nextPresenceUpdate < love.timer.getTime() then
        presence = {
            ...
            largeImageKey = "logo",
            startTimestamp = discordRPC.now,
            partySize = tonumber(party.users),
            partyMax = tonumber(party.max),
        }
