...
function dxDrawText(text, x, y, w, h, color, fontSet, alignX, alignY)
    setColor(color)
    if fontSet then
...
function dxDrawImage(x, y, w, h, image, color, center, rotation)
    if not center then center = {0, 0} end
    local sx, sy = love.graphics.getDimensions()
    if (x >= 0 or x <= sx or y >= 0 or y <= sy) or (x+w >= 0 or y+h >= 0 or x+w <= sx or y+h <= sy) then
        setColor(color)
...
function dxDrawImageSection(x, y, w, h, u, v, usize, vsize, image, color)
    local sx, sy = love.graphics.getDimensions()
    if (x >= 0 and x <= sx and y >= 0 and y <= sy) or (x+w >= 0 and y+h >= 0 and x+w <= sx and y+h <= sy) then
        setColor(color)
        local image = getImageFromCache(image)
        local width, height = image.data:getWidth(), image.data:getHeight()
